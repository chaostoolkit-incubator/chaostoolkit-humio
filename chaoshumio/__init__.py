...
def push_to_humio(
    event: EventPayload, secrets: Secrets, configuration: Configuration
) -> None:
    """
    ...
    """
    token = secrets.get("ingest_token", "").strip()
    if not token:
    ...
    isotimestamp = datetime.fromtimestamp(
        datetime.utcnow().replace(tzinfo=timezone.utc).timestamp(), timezone.utc
    ).isoformat()
    ...
    if "state" in event:
        payload[0]["events"].append(  # type: ignore
            {"timestamp": isotimestamp, "attributes": event["state"]}
        )
    else:
        payload[0]["events"].append(  # type: ignore
            {"timestamp": isotimestamp, "attributes": event["context"]}
        )
